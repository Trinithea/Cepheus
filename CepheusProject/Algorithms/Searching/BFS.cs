...
namespace Cepheus
{
	class BFS : IAlgorithm
	{
		...
		public void Run (Graph<BfsVertex> graph, BfsVertex initialVertex)
		{
			...
			initialVertex.State = StateVertex.States.Open;
			...
			while(queue.Count > 0)
			{
				...
				{
					if (edge.To.State == StateVertex.States.Unvisited)
					{
						edge.To.State = StateVertex.States.Open;
						edge.To.Distance = vertex.Distance + 1;
						edge.To.Predecessor = vertex;
						queue.Enqueue(edge.To);
					}
				}
				vertex.State = StateVertex.States.Closed;
			}
